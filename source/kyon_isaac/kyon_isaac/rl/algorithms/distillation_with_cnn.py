...
from kyon_isaac.rl.modules.student_teacher_with_cnn import StudentTeacherWithCNN 

class Transition(RolloutStorage.Transition):
    latent: torch.Tensor | None = None

class DistillationWithCNN:
    """Distillation algorithm for training a student model to mimic a teacher model."""

    policy: StudentTeacherWithCNN
    ...
    def act(self, obs: TensorDict) -> torch.Tensor:
        ...
        self.transition.latent = self.policy.get_latent(obs)
    ...
    def update(self) -> dict[str, float]:
        ...
        for epoch in range(self.num_learning_epochs):
            ...
            for obs, _, privileged_actions, dones in self.storage.generator():
                ...
                # get student latent
                latent = self.policy.get_latent(obs)

                # Behavior cloning loss
                behavior_loss = self.loss_fn(actions, privileged_actions)

                # Latent cloning loss
                latent_loss = self.latent_loss_fn(self.policy.get_teacher_obs(obs)[:, 3:], latent)

                # Total loss
                loss = loss + behavior_loss + latent_loss
        ...
        loss_dict = {"behavior": mean_behavior_loss, "latent": mean_latent_loss}
