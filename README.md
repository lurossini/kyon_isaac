...
- Throughout the repository, the name `kyon_isaac` only serves as an example and we provide a script to rename all the references to it automatically:

```bash
# Enter the repository
cd IsaacLabExtensionTemplate
# Rename all occurrences of kyon_isaac (in files/directories) to your_fancy_extension_name
...
```bash
python -m pip install -e source/kyon_isaac
```
...
We provide an example UI extension that will load upon enabling your extension defined in `source/kyon_isaac/kyon_isaac/ui_extension_example.py`.
...
```json
{
    "python.analysis.extraPaths": [
        "<path-to-ext-repo>/source/kyon_isaac"
    ]
}
```
